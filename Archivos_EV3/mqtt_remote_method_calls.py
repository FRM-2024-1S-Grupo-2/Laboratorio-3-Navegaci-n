...
#Direccion IP del broker MQTT usado por defecto
mqtt_broker_ip_address = "3e4254d24aca4a2185849d5b0a0487b0.s1.eu.hivemq.cloud"

#Declaracion de numero de indentificacion del robot
Lego_ID = 1

class MqttClient(object):
    def __init__(self, delegate=None):
        #Declaracion del cliente y un delegado opcional de manejo de datos
    ...
    def connect(self, subscription_suffix, publish_suffix, mqtt_broker_ip_address=mqtt_broker_ip_address, lego_robot_number=Lego_ID):
        ...
        #Declaracion de ID del robot y sufijos necesarios para el topico MQTT
        ...
        # Se habilita el TLS para coneccion segura
        self.client.tls_set(tls_version=mqtt.client.ssl.PROTOCOL_TLS)
        self.client.message_callback_add(self.subscription_topic_name, self.on_message)

        
        #Se colocan el nombre de usuario y contrasena para la conexion con el broker MQTT
        #Por razones practicas el nombre del robot sera usado como usuario y contasena
        self.client.username_pw_set(Robot_name,  Robot_name)

        #Se genera la conexion con el broker MQTT en el puerto
    ...
    def send_message(self, function_name, parameter_list=None):
        ...
        #Cracion de diccionario message_dict con una key "type" y valor el nombre de la funcion llamada
        message_dict = {"type": function_name}

        #Revision de si se tiene lista de parametros para la funcion llamada 
        if parameter_list:

            #Verificacion de que los parametros hayan sido ingresados esten en una estructura iterable
        ...
        #Conversion del diccionario message_dict en un mensaje jason
        message = json.dumps(message_dict)

        #Publicacion del mensaje en el broker con el topico de publicacion 
        self.client.publish(self.publish_topic_name, message)

    
    def on_connect(self,client, userdata, flags, rc, properties=None):
        #Funcion de accion al realizarse cuando se genera la conexion al broker

        #Verificacion de varaible de conexion
        if rc == 0:
            print(" ... Connected!")
        else:
            print(" ... Error!!!")
            #Si se presenta un error de conexion descomente la siguente linea y verique que lo causa
        ...
        #Impresion de cual es el topico de publicacion al que se conecto
        print("Publishing to topic:", self.publish_topic_name)

        #Declaracion que la funcion on_subscribe es la misma de la clase cliente
    ...
    def on_publish(self,client, userdata, mid, rc, properties=None):
        #Funcion para cuando la publicacion fue exitosa

        #Cuando el mesaje qos es de confimacion es exitoso se genera la siguiente impresion
        #impresion de identificardor de mensaje recibido
        print("mid: " + str(mid))

    
    def on_subscribe(self, client, userdata, mid, granted_qos, properties=None):
        #Funcion para cuando la suscripcion fue exitosa

        #Si se quiere saber el grado qos y el mesaje de dentificacion descomentar la siguiente linea
        #print("Subscribed: " + str(mid) + " " + str(granted_qos))

        #Impresion del topico al que se ha generado la suscripcion
    ...
    def on_message(self, client, userdata, msg):
        ...
        #Declaracion de message con carga y decodificacion del mensaje recibido
        message = msg.payload.decode()
        
        #Imprime el topico al que se asede el grado qos y el mensaje 
        #print(msg.topic + " " + str(msg.qos) + " " + str(msg.payload))
        
        #Impresion del mensaje recibido por el servidor
        ...
        #Atencion al mensaje recivido y llamado a la funcion apropiada.
        try:
    ...
    def close(self):
        #Funcion para cerrar el cliente MQTT
